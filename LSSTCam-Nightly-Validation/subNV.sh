...
do
  ...
  if [[ ${stepname} == ${firststep} ]]; then
    ...
    do		     
       ...
       export curlog=/sdf/home/l/lsstsvc1/LSSTCam-Nightly-Validation/bps_sub_logs/${stepname}/${bynoext}-log 
...
# comment out to recover from first step timeout
	...
	do
		...
		export bynoext=`echo ${bpsyaml} | cut -d '/' -f 2,2 | cut -d '.' -f 1,1`
#		echo "performing bps submission for step ${stepname} - yaml ${bynoext} "
		export curlog=/sdf/home/l/lsstsvc1/LSSTCam-Nightly-Validation/bps_sub_logs/${ostepname}/${bynoext}-log 
