...
#   alternative list of steps: "step2 step3 step7"
# ------------------------------------------
#subdir=/sdf/data/rubin/shared/campaigns/LSSTCam-Nightly-Validation
#subdir=/sdf/home/l/lsstsvc1/LSSTCam-Nightly-Validation
subdir=`pwd`
...
#export steplist="step1 step2 step3 step7"
#export steplist="step1c-single-visit-tracts step1d-single-visit-global stage3-coadd"
#export steplist="step1a-single-visit-detectors step1b-single-visit-visits step1c-single-visit-tracts step1d-single-visit-global stage3-coadd"
export steplist="NV_1a NV_1b-3"
...
do
  export grplist=`ls bps_${stepname}*.yaml | sed -z "s/\n/ /g"`
  ...
  if [[ ${stepname} == ${firststep} ]]; then
    ...
    do		     
       ...
       export curlog=${subdir}/bps_sub_logs/${stepname}/${bynoext}-log 

# comment out to recover from first step timeout
       time bps submit ${subdir}/${bpsyaml} 2>&1 | tee ${curlog}
	...
	do
		...
		export bynoext=`echo ${bpsyaml} | cut -d '/' -f 2,2 | cut -d '.' -f 1,1`
#		echo "performing bps submission for step ${stepname} - yaml ${bynoext} "
		export curlog=${subdir}/bps_sub_logs/${ostepname}/${bynoext}-log 
		...
		`pipetask report embargo ${qg}  --force-v2 --full-output-filename ./bps_${ostepname}.json  &> pipetask_report_${ostepname}.log &`
	    fi
