...
category=$2
props=$3
cfs cat config/$category/Limits/$props.properties | grep -v "^#" | awk '{print $1}' | sed 's/\/limit..//' | sed 's/\/warn..//' | uniq | xargs -n 1 --replace=CHAN ./chanLims_2.sh \^$1/CHAN\$ | tee $category.dat
